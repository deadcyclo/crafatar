...
var valid_user_id = /^([0-9a-f-A-F-]{32,36}|[a-zA-Z0-9_]{1,16}|img:.+)$/; // uuid|username|img:hash
...
exp.do_avatar_stuff = function(facepath, helmpath, overlay, size, skinpath, callback) {
  ...
  skins.resize_img(filepath, size, function(img_err, image) {
    if (img_err) {
      callback(img_err, -1, null, skinpath);
    } else {
      callback(null, 1, image, skinpath);
    }
  });
};
...
exp.get_avatar = function(rid, userId, overlay, size, callback) {
  ...
  /* TODO: All of this stuff should be shared, so can use for everything */

  if (userId.startsWith('img:')) {
    var skin_h = userId.split(':')[1];
    var skinpath = path.join(config.directories.skins, skin_h + '.png');
    logging.debug(skinpath);
    fs.access(skinpath, function(fs_err) {
      if (fs_err) {
        callback(fs_err, -1, null, skinpath);
      } else {
        var facepath = path.join(config.directories.faces, skin_h + ".png");
        var helmpath = path.join(config.directories.helms, skin_h + ".png");
        fs.access(facepath, function(fs_err) {
          if (fs_err) {
            fs.readFile(skinpath, function(err, img) {
              if (err) {
                callback(err, -1, null, skinpath);
              } else {
                skins.extract_face(img, facepath, function (err2) {
                  if (err2) {
                    callback(err2, -1, null, skinpath);
                  } else {
                    logging.debug(rid, "face extracted");
                    skins.extract_helm(rid, facepath, img, helmpath, function (err3) {
                      if (!err3) {
                      ...
                      exp.do_avatar_stuff(facepath, helmpath, overlay, size, skinpath, callback);
                    });
                  }
                });
              }
